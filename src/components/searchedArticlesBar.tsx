...
export default function SearchedArticlesBar({ searchedTerm }: { searchedTerm: string }) {
    ...
    <Box sx={{ 
      ...
      <Box sx={{
        ...
        <Typography variant="h1" sx={{ color: theme.palette.text.primary, fontSize: '1.5rem', fontWeight: '700' }}>
          Hľadaný výraz v článkoch: {searchedTerm}
        </Typography>
      </Box>
      <Box sx={{ position: 'relative' }}>
        <Search>
          ...
          <StyledInputBase
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') handleArticleSearch(); // Trigger search on Enter key
            }}
          />
        </Search>
      </Box>
